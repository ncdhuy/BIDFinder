...
BIDFINDER_TYPESENSE_CHECKPOINT="${BIDFINDER_TYPESENSE_CHECKPOINT:-}"
BIDFINDER_TYPESENSE_PROVENANCE="${BIDFINDER_TYPESENSE_PROVENANCE:-}"
BIDFINDER_SERVING_REPORT_PATH="${BIDFINDER_SERVING_REPORT_PATH:-}"
BIDFINDER_SERVING_MARKDOWN_PATH="${BIDFINDER_SERVING_MARKDOWN_PATH:-}"
...
BIDFINDER_SERVING_GENERATION="${BIDFINDER_SERVING_GENERATION:-}"
...
require_serving_state() {
  local path report_generation
  for path in "$BIDFINDER_TYPESENSE_CHECKPOINT" "$BIDFINDER_TYPESENSE_PROVENANCE" "$BIDFINDER_SERVING_REPORT_PATH"; do
    [[ -n "$path" && -f "$path" ]] || { echo "missing serving state: $path" >&2; return 2; }
  done
  [[ -n "$BIDFINDER_SERVING_GENERATION" ]] || { echo "serving generation is not configured" >&2; return 2; }
  [[ "$BIDFINDER_TYPESENSE_SERVING_GENERATION" == "$BIDFINDER_SERVING_GENERATION" ]] || {
    echo "Typesense/serving generation mismatch: $BIDFINDER_TYPESENSE_SERVING_GENERATION != $BIDFINDER_SERVING_GENERATION" >&2
    return 2
  }
  report_generation="$($BIDFINDER_PYTHON - "$BIDFINDER_SERVING_REPORT_PATH" <<'PY'
import json
import sys
from pathlib import Path
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(str(payload.get("serving_generation", "")).strip())
PY
)"
  [[ "$report_generation" == "$BIDFINDER_SERVING_GENERATION" ]] || {
    echo "serving generation/report mismatch: $BIDFINDER_SERVING_GENERATION != $report_generation" >&2
    return 2
  }
}
