...
class CheckpointStore:
    """Durable state keyed by source key and official calendar date."""
    ...
    def get(self, source_key: str, partition_date: str) -> Checkpoint | None:
        row = self._connection.execute(
            "SELECT * FROM ingestion_checkpoint WHERE source_key=? AND partition_date=?",
            (source_key, partition_date),
        ).fetchone()
        if row is None:
            return None
        return Checkpoint(
            source_key=row["source_key"], partition_date=row["partition_date"],
    ...
    def ensure(self, source_key: str, partition_date: str) -> Checkpoint:
        """Create visible PENDING state without claiming the partition."""

        now = self._now()
        self._connection.execute(
            """INSERT INTO ingestion_checkpoint
               (source_key, partition_date, status, attempt_count, engine_version,
                schema_version, updated_at)
               VALUES (?, ?, 'PENDING', 0, ?, ?, ?)
               ON CONFLICT(source_key, partition_date) DO NOTHING""",
            (source_key, partition_date, ENGINE_VERSION, SCHEMA_VERSION, now),
        )
        self._connection.commit()
        return self.get(source_key, partition_date)  # type: ignore[return-value]

    def start(self, source_key: str, partition_date: str, *, force: bool = False) -> Checkpoint:
        self.ensure(source_key, partition_date)
        current = self.get(source_key, partition_date)
        ...
        self._connection.execute(
            """INSERT INTO ingestion_checkpoint
               (source_key, partition_date, status, attempt_count, started_at,
            ...
            (source_key, partition_date, attempt, now, ENGINE_VERSION, SCHEMA_VERSION, now),
        )
        self._connection.commit()
        return self.get(source_key, partition_date)  # type: ignore[return-value]

    def update_metrics(self, source_key: str, partition_date: str, **metrics: Any) -> None:
        allowed = {
        ...
        self._connection.execute(
            f"UPDATE ingestion_checkpoint SET {assignments}, updated_at=? WHERE source_key=? AND partition_date=?",
            (*[values[key] for key in metrics if key in allowed], values["updated_at"], source_key, partition_date),
        )
        self._connection.commit()

    def finish(self, source_key: str, partition_date: str, status: IngestionStatus, **metrics: Any) -> Checkpoint:
        ...
        self.update_metrics(source_key, partition_date, **metrics)
        now = self._now()
        self._connection.execute(
            "UPDATE ingestion_checkpoint SET status=?, completed_at=?, updated_at=? WHERE source_key=? AND partition_date=?",
            (status.value, now, now, source_key, partition_date),
        )
        self._connection.commit()
        return self.get(source_key, partition_date)  # type: ignore[return-value]

    def fail(self, source_key: str, partition_date: str, code: str, message: str, *, quarantine: bool = False, **metrics: Any) -> Checkpoint:
        current = self.get(source_key, partition_date)
        if current is None:
        ...
        now = self._now()
        self._connection.execute(
            "UPDATE ingestion_checkpoint SET status=?, last_error_code=?, last_error_message=?, updated_at=? WHERE source_key=? AND partition_date=?",
            (status.value, code, message[:2000], now, source_key, partition_date),
        )
        self._connection.commit()
        return self.get(source_key, partition_date)  # type: ignore[return-value]
