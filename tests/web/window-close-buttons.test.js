...
assert.match(style, /\.btn-close,\s*\.window-close-button\s*\{[\s\S]*?width:\s*36px;[\s\S]*?height:\s*36px;[\s\S]*?border-radius:\s*8px;/i);
assert.match(style, /\.btn-close:hover,\s*\.window-close-button:hover\s*\{[\s\S]*?border-color:\s*#c43d3d;[\s\S]*?color:\s*#c43d3d;/i);
assert.match(style, /\.btn-close:hover svg,\s*\.window-close-button:hover svg\s*\{[\s\S]*?color:\s*inherit;/i);
assert.match(style, /\.btn-close:focus-visible,\s*\.window-close-button:focus-visible\s*\{/i);
