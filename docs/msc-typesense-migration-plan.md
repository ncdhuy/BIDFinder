...
- Numeric values remain numeric. Quantity maps from `khoiLuongDouble` or `soLuong`; prices map from the verified source price field; fractional `soNhaThauThamDu` remains the source numeric `bidder_count`; year ranges such as `2024-2025` remain textual production-year values.
